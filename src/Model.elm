module Model exposing (create, set, combine, run, map, andThenDo, insteadDo, modify)

{-| Helper functions for mapping and manipulating Tupels of (Model, Cmd).

# Initializing composed models
@docs create, set, combine, run, andThenDo, insteadDo
...
{-| Modify an existing (model, Cmd) tuple with a model transforming function. The Cmd
will not be altered.
-}
...
{-| Add a command to a (model, Cmd) tuple.
-}
...
{-| Replaces the command of a (model, Cmd) tuple.
-}
